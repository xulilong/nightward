...
export default function Home() {
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => {
      ...
      if (!event.repeat && ['A','S','D','Q','W','F','X',' '].includes(key)) {
  ...
  return <main ref={shell} className="game-shell">
    ...
    <div className="stage-wrap"><section className="stage" aria-label="逐夜之刃游戏区域" data-chapter={state.stage}>
      <canvas ref={canvas} width={1280} height={720} tabIndex={0} aria-label="方向键移动，A 连斩，X 剑气，D 闪避，S 基础防御，第二章 Q 突进，W 治疗，F 交互，R 暂停"/>
      {state.mode !== 'menu' && <>
        <div className="in-game-controls">{iconButtons}</div>

        <div className="player-hud"><div className="player-badge"><Flame/></div><div className="player-bars"><div className="player-label"><b>烬</b><strong>{Math.ceil(state.hp)}<small> / 100</small></strong></div><div className="health-track"><div style={{width: `${state.hp}%`}}/></div>{<div className="energy-row" aria-label="剑气刃能，近战命中恢复">{[0,1,2].map(i=><span key={i} className={i < state.energy ? 'charged' : ''}/>)}<small>X 剑气 · 普攻命中充能</small></div>}</div></div>
        ...
        <div className="skillbar">{[
          ...
          {key:'X', name:'剑气', icon:<Wind/>, cd:state.energy<=0?-1:(game.current?.player.atk??0), action:'ranged'},
        ...
        ].map(skill=><Button key={skill.key} className={`skill ${skill.cd > 0 ? 'cooling' : ''} ${skill.cd === -2 ? 'skill-locked' : ''}`} data-held={!!heldKey[skill.key]} variant="ghost" aria-label={`${skill.name}${skill.cd===-2?'，第二章解锁':skill.cd===-1?'，刃能不足，普攻命中充能':skill.cd>0?'，冷却中':''}`} aria-keyshortcuts={skill.key} disabled={state.mode !== 'playing' || skill.cd !== 0} onClick={()=>game.current?.action(skill.action)}>
          <kbd>{skill.key}</kbd><span className="skill-emblem">{skill.icon}
            {skill.cd>0&&<i className="skill-cooldown" aria-hidden="true" style={{transform:`scaleY(${Math.min(1,skill.cd/(skill.key==='X'?.52:skill.key==='Q'?(game.current?.powers.has(9)?4:5):skill.key==='S'?.8:(game.current?.powers.has(2)?.85:1.2)))})`}}/>}
            {skill.cd>0&&<b className="skill-countdown" aria-hidden="true">{skill.cd.toFixed(1)}</b>}
            {skill.cd===-2&&<Lock className="skill-lock-mark" aria-hidden="true"/>}
          </span><span className="skill-name">{skill.cd===-2?'第二章解锁':skill.cd===-1?'普攻充能':skill.name}</span>
        </Button>)}</div>
      </>}
      {state.mode === 'menu' && <div className="splash-screen" data-motion-paused={pageHidden || chapterMenu}>
        ...
        <div className="splash-actions">
          ...
          {loadError ? <><p className="load-error">场景未能载入，请重新尝试。</p><Button className="start-button" onClick={()=>window.location.reload()}>重新加载</Button></> : ready ? <><Button className="enter-mist-button" data-held={!!heldKey.F} onClick={()=>state.hasSave?game.current?.continueCampaign():start()} aria-keyshortcuts="F" aria-describedby="enter-mist-hint"><span className="entry-ornament" aria-hidden="true"/><span>{state.hasSave ? '继续旅程' : '踏入长夜'}</span><span className="entry-ornament" aria-hidden="true"/></Button><div className="entry-key-hint" id="enter-mist-hint">点击启程<span aria-hidden="true">·</span>或按 <kbd>F</kbd></div><div className="campaign-menu"><span>雾林边境 / 灰烬村庄 · 前两章开放</span><small>新旅程难度</small><div className="difficulty-switch" aria-label="游戏难度"><button aria-pressed={state.difficulty==='story'} onClick={()=>setDifficulty('story')}>轻松</button><button aria-pressed={state.difficulty==='normal'} onClick={()=>setDifficulty('normal')}>标准</button><kbd>A / D</kbd></div>{state.hasSave&&<Button variant="ghost" className="continue-button" onClick={start}>重新启程 · 新战役</Button>}</div><p className="entry-controls">方向键移动 · A 连斩 · X 剑气 · D 闪避 · S 防御<br/><small>第二章 Q 突进　 W 治疗　 F 交互　 R 暂停</small></p></> : <><div className="loading-track"><span style={{width:`${loading}%`}}/></div><p>正在点亮沿途灯火 <span>{loading}%</span></p></>}
        </div>
      </div>}
