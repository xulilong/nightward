...
import { Flame, Swords, Volume2, VolumeX, Maximize, Pause, Play, ArrowUpRight, RotateCcw, Wind, Sparkles, Crosshair, ChevronRight, Heart, Compass, Shield, Lock } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => {
      ...
      if (!event.repeat && ['Z','S','C','Q','A','F',' '].includes(key)) {
        setHeldKey(keys => ({...keys, [key === ' ' ? 'C' : key]: true}));
      }
    };
    const up = (event: KeyboardEvent) => {
      const key = event.key === ' ' ? 'C' : event.key.toUpperCase();
  ...
  return <main ref={shell} className="game-shell">
    ...
    <div className="stage-wrap"><section className="stage" aria-label="逐夜之刃游戏区域" data-chapter={state.stage}>
      <canvas ref={canvas} width={1280} height={720} tabIndex={0} aria-label="方向键移动，Z 连斩，C 闪避，S 基础防御，第二章 Q 突进，A 治疗，F 交互，R 暂停"/>
      {state.mode !== 'menu' && <>
        <div className="in-game-controls">{iconButtons}</div>
        <div className="in-world-hint">↑↓←→ 移动 <span>·</span> {state.aimMode === 'auto' ? '自动瞄准' : '鼠标瞄准'}</div><div className="area-name"><small>{currentChapter.subtitle}</small><span>{state.stage===0?['追踪小径','溪流哨桥','废弃哨所','骑士营地'][state.chapter]:(villageDistricts.find(d=>state.playerY>=d.from&&state.playerY<d.to)?.name??'灰烬村庄')}</span></div>
        <div className="player-hud"><div className="player-badge"><Flame/></div><div className="player-bars"><div className="player-label"><b>烬</b><span>巡灯人 · {state.difficulty === 'story' ? '轻松' : '标准'}</span><strong>{Math.ceil(state.hp)}<small> / 100</small></strong></div><div className="health-track"><div style={{width: `${state.hp}%`}}/></div><div className="energy-row">{[0,1,2].map(i=><span key={i} className={i < state.energy ? 'charged' : ''}/>)}<small>刃能 · 近战命中充能</small></div></div></div>
        <div className="quest"><small>{currentChapter.name} · 当前目标</small><strong><span>◇</span> {state.objective}</strong><div className="quest-progress"><i style={{width:`${state.progress}%`}}/></div><p>已击败 {state.kills} <span>·</span> {Math.floor(state.time/60).toString().padStart(2,'0')}:{Math.floor(state.time%60).toString().padStart(2,'0')}</p></div>
        <div className="route-map" aria-label="驿道路线进度">
          <div className="route-map-title"><Compass size={13}/><span>灯火之路</span><b>{state.stage+1} / 8</b></div>
          ...
          {state.stage===1&&<div className="rescue-ledger">{sitesFor(1).map((site,i)=><div key={site.id} className={state.completedSites.includes(site.id)?'done':''}><span>{state.completedSites.includes(site.id)?'✓':'◇'}</span><p>{site.name}<small>{state.completedSites.includes(site.id)?'已完成 · 火势受控':['淬刃 · 伤害提升','村民脱险 · 生命恢复','保住粮食 · 药剂补给'][i]}</small></p></div>)}</div>}
          <small>强化 {state.upgradeCount} · 精准闪避 {state.perfectDodges}</small>
        </div>
        <Button className="heal-button" data-held={!!heldKey.A} variant="outline" disabled={state.mode!=='playing'||state.potions===0||state.hp>=100} onClick={()=>game.current?.action('heal')}><kbd>A</kbd><Heart size={16}/><span>灯火药剂 <b>× {state.potions}</b></span></Button>
        ...
        {state.guidance&&!state.toast&&!state.interact&&state.mode==='playing'&&<div className="context-guidance">{state.guidance}</div>}
        ...
        <div className="skillbar">{[
          {key:'Z', name:'三连斩', icon:<Swords/>, cd:0, action:'attack'},
      ...
      {state.mode === 'menu' && <div className="splash-screen" data-motion-paused={pageHidden || chapterMenu}>
        ...
        <div className="splash-actions">
          ...
          {loadError ? <><p className="load-error">场景未能载入，请重新尝试。</p><Button className="start-button" onClick={()=>window.location.reload()}>重新加载</Button></> : ready ? <><Button className="enter-mist-button" data-held={!!heldKey.F} onClick={()=>state.hasSave?game.current?.continueCampaign():start()} aria-keyshortcuts="F" aria-describedby="enter-mist-hint"><span className="entry-ornament" aria-hidden="true"/><span>{state.hasSave ? '继续旅程' : '踏入长夜'}</span><span className="entry-ornament" aria-hidden="true"/></Button><div className="entry-key-hint" id="enter-mist-hint">点击启程<span aria-hidden="true">·</span>或按 <kbd>F</kbd></div><div className="campaign-menu"><span>雾林边境 / 灰烬村庄 · 前两章开放</span><small>新旅程难度</small><div className="difficulty-switch" aria-label="游戏难度"><button aria-pressed={state.difficulty==='story'} onClick={()=>setDifficulty('story')}>轻松</button><button aria-pressed={state.difficulty==='normal'} onClick={()=>setDifficulty('normal')}>标准</button><kbd>A / D</kbd></div>{state.hasSave&&<Button variant="ghost" className="continue-button" onClick={start}>重新启程 · 新战役</Button>}</div><p className="entry-controls">方向键移动 · Z 连斩 · C 闪避 · S 防御<br/><small>第二章 Q 突进　 A 治疗　 F 交互　 R 暂停</small></p></> : <><div className="loading-track"><span style={{width:`${loading}%`}}/></div><p>正在点亮沿途灯火 <span>{loading}%</span></p></>}
        </div>
      </div>}
      ...
      {state.mode === 'paused' && <div className="modal-backdrop"><div className="modal"><Flame className="modal-symbol"/><div className="eyebrow">TAKE A BREATH</div><h2>灯火为你停留</h2><p>存档在营地与强化后自动保存。准备好了就继续上路。</p><Button className="start-button" onClick={()=>game.current?.togglePause()}><Play/>继续冒险 <kbd>F</kbd></Button><Button variant="ghost" onClick={restartChapter}><RotateCcw/>重新开始本章 <kbd>X</kbd></Button><div className="pause-help">↑↓←→ 移动 · Z 连斩 · C 闪避 · S 防御<br/>第二章 Q 突进 · A 治疗 · F 交互 · R 暂停<br/>键盘攻击自动瞄准 · 鼠标点击切换自由瞄准</div></div></div>}
