...
export default function Home() {
  ...
  return <main ref={shell} className="game-shell">
    ...
    <div className="stage-wrap"><section className="stage" aria-label="逐夜之刃游戏区域">
      <canvas ref={canvas} width={1280} height={720} tabIndex={0} aria-label="使用 WASD 移动，鼠标瞄准，Z 或左键挥剑，X 或右键剑气，C 闪避，Q 突进，E 回旋斩，F 交互，R 暂停"/>
      {state.mode !== 'menu' && <>
      ...
      {state.mode === 'menu' && <div className="intro-overlay"><div className="intro-content"><div className="eyebrow"><span/> A LIGHT AGAINST THE DARK</div><h1>逐夜之刃</h1><h2>雾林驿道</h2><p>长夜将至，最后一座城市仍在迁徙。<br/>握紧你的剑，为归途点亮下一盏灯。</p><Button className="start-button" onClick={start}><Swords/>踏入雾林<ArrowUpRight/></Button><div className="intro-keyboard"><kbd>W A S D</kbd> 移动 <kbd>Z</kbd> 连斩 <kbd>X</kbd> 剑气</div><div className="intro-notes"><span>单人冒险</span><i/>约 5–8 分钟<i/><span>F 开始 · 左手即可操作</span></div></div><div className="intro-caption"><span>01 — THE MISTWOOD</span><p>灯火所至，即是归途。</p></div><div className="version-tag">PLAYABLE PROTOTYPE <span>v0.1</span></div></div>}
      {state.mode === 'paused' && <div className="modal-backdrop"><div className="modal"><Flame className="modal-symbol"/><div className="eyebrow">TAKE A BREATH</div><h2>灯火为你停留</h2><p>冒险已暂停。准备好了就继续上路。</p><Button className="start-button" onClick={()=>game.current?.togglePause()}><Play/>继续冒险 <kbd>F</kbd></Button><Button variant="ghost" onClick={start}><RotateCcw/>重新开始本关 <kbd>X</kbd></Button><div className="pause-help">WASD 移动 · Z 连斩 · X 剑气 · C 闪避<br/>Q 突进 · E 回旋斩 · F 交互 · R 暂停<br/>键盘攻击自动瞄准 · 鼠标点击切换自由瞄准</div></div></div>}
      ...
      <div className="touch-controls"><div className="dpad">{[['w','↑'],['a','←'],['s','↓'],['d','→']].map(([key,label])=><Button key={key} variant="outline" aria-label={`移动${label}`} onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);game.current?.setKey(key,true);}} onPointerUp={()=>game.current?.setKey(key,false)} onPointerCancel={()=>game.current?.setKey(key,false)}>{label}</Button>)}</div></div>
    </section></div>
    <footer className="bottom-bar"><div><span className="live-dot"/>冒险试玩版<span className="footer-divider">/</span><span>第一章 · 雾林驿道</span></div><p><kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> 移动 <span>·</span> Z 连斩 / X 剑气 / C 闪避 <span>·</span> R 暂停</p><span className="footer-motto"><Flame/>循灯而行</span></footer>
